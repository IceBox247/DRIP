...
takes the trade fee, keeps its protocol cut, and pays our **creator share in USDG**. We collect it by
...
at launch (see SPEC §2.1, DECISIONS.md #5). Everything we build operates on that USDG revenue *after*
...
| Launchpad | **Pons** (external) — collects the trade fee, keeps its protocol cut, pays our creator share in USDG to our payout wallet | *not ours* | — |
| Contract | `DripToken` — plain ERC-20, no fee logic (may be Pons-deployed) | `contracts/src/DripToken.sol` | 1 |
| Contract | `FeeDistributor` — splits the **USDG creator fees received from Pons** → ~2/3 auto-buy / ~1/3 marketing. Optional on-chain; can live in the keeper | `contracts/src/FeeDistributor.sol` | 1 |
...
| Automation | Keeper (Gelato / Chainlink Automation) — hourly: read USDG payout wallet, distribute, swap auto-buy USDG → Stock Token, run cycle | `keeper/` | 3 |
...
│  trading venue   │  keeps its protocol cut; pays our creator share in USDG
└────────┬─────────┘
         │  USDG creator fees, via Pons automation (push)
         ▼
┌──────────────────┐
│  Payout wallet   │  a wallet WE designate; Pons automation routes USDG here
└────────┬─────────┘
         │  keeper reads USDG balance
         ▼
┌──────────────────┐
│ FeeDistributor   │  split (on-chain OR in keeper), of received USDG:
│  (or keeper)     │    ~2/3 → auto-buy buffer (USDG)
└────────┬─────────┘    ~1/3 → marketing/ops treasury
         │              (Pons already took its cut — nothing forwarded back)
   auto-buy buffer (USDG)
         │
         ▼
┌──────────────────────────────────┐
│ Keeper (hourly)                   │  swap USDG → Stock Token(s), slippage-bounded
└────────┬──────────────────────────┘
         ▼
┌────────────────┐
...
- **Push mode (Pons default for us):** enable Pons automation to route creator fees (USDG) to our
  payout wallet. The keeper just reads that USDG balance — no privileged keys needed.
...
Either way the keeper ends up with a known **USDG** balance to split + swap. Keep the Pons
integration isolated in one module (`keeper/`), with everything downstream (`FeeDistributor`,
`ReserveManager`, `RewardVault`) launchpad-agnostic — they just see "USDG in, Stock Token out."
...
`read USDG → distribute → swap → cycle` path. Design on-chain entrypoints to be **permissionless-safe**
