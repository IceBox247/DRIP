...
> confirming Pons's fee economics + delivery and distributing the USDG we net.

- [ ] Confirm Pons fee params on-chain: total fee, creator/protocol split, currency (USDG), and that
      a fee high enough to net ~3% is allowed + worth the volume hit (DECISIONS.md #5, BLOCKERS #3).
- [ ] Enable Pons automation → route creator fees (USDG) to our payout wallet (push).
- [ ] Confirm who deploys DRIP — Pons or us (DECISIONS.md #6). If us: `DripToken` (plain ERC-20,
      fair launch, no team allocation).
- [ ] `FeeDistributor` (split received USDG → ~2/3 auto-buy / ~1/3 marketing), **or** decide to do
...
**Exit gate:** Pons fee economics + delivery confirmed; our USDG share is received and split correctly
on testnet.
...
- [ ] Auto-buy: swap the auto-buy USDG → Stock Token(s) (slippage-bounded).
