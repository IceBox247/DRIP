...
0. **Receive fee** — Pons collects the trade fee, keeps its protocol cut, and pays our creator share
   in **ETH**. The keeper gets it via the **fee-source adapter** (`config/constants.json` feeSource):
   - *push mode (Pons default for us)*: read the ETH balance Pons automation credited to our payout
     wallet. No privileged keys.
   - *pull mode (fallback)*: call Pons's claim entrypoint to collect accrued ETH fees, then
     distribute.
...
1. **Distribute** — split the received ETH → ~2/3 auto-buy buffer, ~1/3 marketing/ops. Via
   `FeeDistributor.distribute()` on-chain, or directly here. (No launchpad forwarding — Pons already
   took its cut.)
2. **Auto-buy** — swap the auto-buy ETH into the target Stock Token(s). Swaps must be
...
- **Isolate fee access.** With Pons **push** automation the keeper just reads the ETH payout wallet —
...
- **Note on cadence.** Pons runs a per-token vault (~every 5 min) that processes fee claims. Our
  reward cycle is hourly — the keeper batches whatever ETH has accrued since the last cycle.
